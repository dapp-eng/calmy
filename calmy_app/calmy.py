...
class RecapScreen(MDScreen):
    ...
    def generate_pdf(self):
        # Lokasi file pdf ke folder assets untuk disimpan sementara
        app = App.get_running_app()
        pdf_path = app.resource_path("assets/weekly_recap.pdf")

        # Pembuatan file pdf dengan fpdf
        pdf = FPDF()
        pdf.set_auto_page_break(auto=True, margin=15)
        
        # Memasukkan tiap grafik ke file pdf
        for path in [self.weight_path, self.height_path, self.kalori_path]:
            pdf.add_page()
            pdf.image(path, x=10, y=10, w=190)  # Menyesuaikan ukuran gambar

        # Simpan pdf ke folder assets
        pdf.output(pdf_path)
        return pdf_path
    ...
    def download_pdf(self):
        ...
        if platform == 'win' or platform == 'linux' or platform == 'macosx':
            # Untuk menyesuaikan platform user apakah windows/linux/macosx
            user_home = os.path.expanduser("~")
            download_folder = os.path.join(user_home, "Downloads") # Menyimpan file pdf ke folder Downloads
            
            if not os.path.exists(download_folder): # Jaga-jaga kalau folder Downloads tidak ada
            ...
            self.show_info(final_path) # Menampilkan pop up nya
        else:
            print(f"Platform {platform} tidak dikenali untuk pengunduhan PDF.")
        
    def show_info(self, pdf_path):
        # Menampilkan popup jika pdf berhasil disimpan
        app = App.get_running_app()
        dialog = MDDialog(
            MDDialogIcon(
                icon="check-circle-outline",
...
class ReviewScreen(MDScreen):
    ...
    def on_pre_enter(self):
        # Ambil data dari database saat screen akan dimasuki
    ...
    def show_weekly_review(self):
        ...
        if username:
            ...
            if result and result[0]:
                ...
                self.review_text = week_review  # Update text di MDLabel
...
class Calmy(MDApp):
    ...
    def on_start(self):
        ...
        Clock.schedule_interval(self.calculate_bmi, 0.5)
        Clock.schedule_interval(self.calculate_bmr, 0.5)
